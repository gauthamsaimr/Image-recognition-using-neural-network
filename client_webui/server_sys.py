...
def set_camera_options(camera):
    ...
    if resolution:
        ...
        #     resolution['height']
        # )
        camera.set(3, resolution['width'])
        camera.set(4, resolution['height'])
    ...
    # Set shutter speed.
    if shutter_speed:
        camera.set(15, shutter_speed) # Exposure
...
def capture_b64(camera):
    r, img = camera.read()
    r, buff = cv2.imencode('.jpg', img)
    enc =  base64.b64encode(buff)
    return enc
...
def getimage_camera():
    """
    """
    r = make_response(capture_b64(camera))
